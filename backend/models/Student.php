...
class Student {
    ...
    // Les profils liés à un compte supprimé restent en base pour préserver
    // les historiques scolaires, mais ne doivent plus apparaître dans la
    // gestion quotidienne.
    ...
    // Read a single student's profile by their user_id (used when a student
    // fetches their own record via session, since $_SESSION['user_id']
    // refers to users.id, not students.id)
    ...
    // Read all students with enrollment info (for dashboard, etc.)
    ...
    /**
     * Préserve NULL pour les champs facultatifs. Une chaîne vide envoyée à
     * une colonne DATE ou ENUM échoue avec MySQL en mode strict.
     */
    private function sanitizeForWrite() {
        $this->user_id = (int)$this->user_id;
        $this->student_number = $this->cleanRequired($this->student_number);
        ...
        $this->gender = $this->cleanNullable($this->gender);
        $this->nationality = $this->cleanNullable($this->nationality);
        $this->emergency_contact_name = $this->cleanNullable($this->emergency_contact_name);
        $this->emergency_contact_phone = $this->cleanNullable($this->emergency_contact_phone);
