...
const Students = () => {
  ...
  useEffect(() => {
    ...
    const mockSpecializations = [
      ...
      { id: 2, name: "Mathématiques", capacity: 35 },
  ...
  const checkCapacity = (levelName, specializationName) => {
    ...
    const currentEnrollments = students.filter(
      (s) => s.level === level.name && s.specialization === specialization.name,
    ).length;
    ...
    if (currentEnrollments >= maxCapacity) {
      return {
        available: false,
        message: `Capacité maximale (${maxCapacity}) atteinte pour ${levelName} - ${specializationName}.`,
      };
    }

    return {
      available: true,
      message: `Places disponibles: ${maxCapacity - currentEnrollments}/${maxCapacity}`,
    };
  };
  ...
  const handleSubmit = () => {
    if (modalType === "add") {
      if (!capacityInfo.available) {
        error(
          "Impossible de créer l'étudiant, la capacité de la classe est atteinte.",
        );
        return;
      }
      const newStudent = {
              ...
              >
                <td className="p-4">
                  <div>
                    ...
                    {/* <div className="text-xl text-gray-500">{student.phone}</div> */}
                  </div>
                </td>
                ...
                <td className="p-4">
                  <span className="bg-blue-100 text-blue-800 px-3 py-1 rounded-full text-xl font-semibold">
                    {student.level}
                  </span>
                </td>
                <td className="p-4 text-gray-600 dark:text-gray-400">
                ...
                <td className="p-4">
                  <span
                    className={`px-6 py-2 rounded-full text-xl font-semibold ${student.status === "Active" ? "bg-green-100 text-green-800" : "bg-red-100 text-red-800"}`}
                  >
                    {student.status}
                  </span>
                </td>
        ...
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
          <div className="bg-white dark:bg-gray-800 rounded-lg p-6 w-full max-w-5xl max-h-[90vh] overflow-y-auto">
            <h2 className="text-2xl font-bold mb-4">
              {modalType === "add" && "Nouveau étudiant"}
              {modalType === "edit" && "Modifier étudiant"}
              {modalType === "view" && "Détails de l'étudiant"}
              {modalType === "enroll" && "Gérer inscription"}
            </h2>
            ...
            {modalType === "view" && selectedStudent && (
              <div className="space-y-4">
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  ...
                  <div>
                    <label className="block text-2xl font-bold text-gray-700 dark:text-gray-300">
                      Email
                    </label>
                    <p className="text-gray-900 text-2xl font-medium">
                  ...
                  <div>
                    ...
                    <span
                      className={`px-3 py-1 rounded-full text-2xl font-medium ${selectedStudent.status === "Active" ? "bg-green-100 text-green-800" : "bg-red-100 text-red-800"}`}
                    >
            ...
            {modalType === "enroll" && selectedStudent && (
              ...
              >
                <div className="bg-blue-50 text-blue-800 p-4 rounded-lg mb-4">
                  <h3 className="font-semibold text-2xl text-gray-700 dark:text-gray-300 mb-2">
                    {selectedStudent.firstName} {selectedStudent.lastName}
                  </h3>
                  <p className="text-xl font-semibold text-gray-700 dark:text-gray-300">
                    Numéro: {selectedStudent.studentNumber}
                  </p>
                </div>
                <div className="grid grid-cols-2 gap-8">
                  <div>
                    <label className="block text-2xl font-medium text-gray-700 dark:text-gray-300 mb-1">
                      Nouveau Niveau
                    </label>
                    <select
                      name="level"
                      defaultValue={selectedStudent.level}
                      className="w-full border dark:border-gray-700 rounded-lg px-3 py-2 focus:ring-2 focus:ring-blue-500 focus:border dark:border-gray-700-blue-500"
                    >
                ...
                <div className="flex justify-end space-x-3 pt-4  mt-4">
                  ...
                  >
                    <Save size={16} /> Mettre à jour l'inscription
                  </button>
                </div>
              </form>
            )}
            ...
            {(modalType === "add" || modalType === "edit") && (
              ...
              >
                <div className="grid grid-cols-2 gap-8">
                  <div>
                    ...
                    <input
                      ...
                      required
                      className="w-full border dark:border-gray-700 rounded-lg px-2 py-2 focus:ring-2 focus:ring-blue-500 focus:border dark:border-gray-700-blue-500"
                    />
                  </div>
                  <div>
                    <label className="block text-2xl font-medium text-gray-700 dark:text-gray-300 mb-1">
                ...
                {modalType === "add" &&
                  ...
                  formData.specialization && (
                    <div
                      className={`mt-4 p-3 rounded-lg flex items-center text-sm ${capacityInfo.available ? "bg-green-50 text-green-800" : "bg-red-50 text-red-800"}`}
                    >
                ...
                <div className="flex justify-end space-x-3 pt-4 ">
                  ...
                  <button
                    type="submit"
                    disabled={modalType === "add" && !capacityInfo.available}
