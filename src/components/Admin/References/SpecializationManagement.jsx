...
const SpecializationManagement = () => {
  ...
  const load = useCallback(async () => {
    try {
      const res = await apiService.getSpecializations();
      setItems(res.data || []);
    } catch (err) {
      error('Erreur de chargement des spécialisations');
    }
  }, [error]);
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      if (editing) {
        await apiService.updateSpecialization(editing.id, { name: name.trim(), capacity });
        success('Spécialisation modifiée');
      } else {
        await apiService.createSpecialization({ name: name.trim(), capacity });
        success('Spécialisation ajoutée');
      }
      setShowModal(false);
      load();
    } catch (err) {
  ...
  const handleDelete = async (id) => {
    if (!window.confirm('Supprimer cette spécialisation ?')) return;
    try {
      await apiService.deleteSpecialization(id);
      success('Spécialisation supprimée');
      load();
    } catch (err) {
      error(err.message || 'Erreur');
    }
  ...
  return (
    <div>
      <div className="flex justify-between items-center mb-4">
        <h2 className="text-2xl font-bold">Gestion des spécialisations</h2>
        <button
      ...
      <div className="bg-white dark:bg-gray-800 rounded shadow overflow-hidden">
        ...
        {items.length === 0 && (
          <div className="p-6 text-center text-gray-500">Aucune spécialisation</div>
        )}
      </div>
      ...
      {showModal && (
        <div className="app-modal-layer bg-black bg-opacity-50">
          <div className="bg-white dark:bg-gray-800 rounded-lg p-6 w-full max-w-md">
            <h3 className="text-xl font-bold mb-4">{editing ? 'Modifier' : 'Ajouter'} une spécialisation</h3>
