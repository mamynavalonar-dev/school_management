...
const BuildingManagement = () => {
  ...
  const load = useCallback(async () => {
    try {
      ...
      error('Erreur de chargement des bâtiments');
    }
  }, [error]);
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      if (editing) {
        await apiService.updateBuilding(editing.id, { name: name.trim() });
        success('Bâtiment modifié');
      } else {
        await apiService.createBuilding({ name: name.trim() });
        success('Bâtiment ajouté');
      }
      setShowModal(false);
      load();
    } catch (err) {
  ...
  return (
    <div>
      ...
      <div className="bg-white dark:bg-gray-800 rounded shadow overflow-hidden">
        ...
        {items.length === 0 && (
          <div className="p-6 text-center text-gray-500">Aucun bâtiment</div>
        )}
      </div>
      ...
      {showModal && (
        <div className="app-modal-layer bg-black bg-opacity-50">
          <div className="bg-white dark:bg-gray-800 rounded-lg p-6 w-full max-w-md">
            <h3 className="text-xl font-bold mb-4">{editing ? 'Modifier' : 'Ajouter'} un bâtiment</h3>
