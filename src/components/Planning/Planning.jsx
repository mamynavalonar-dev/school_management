...
const Planning = () => {
  ...
  // Référentiels (données de simulation)
  ...
  const [teachers] = useState([
    { id: 1, name: "Pierre Durand" },
    { id: 2, name: "Marie Leblanc" },
    { id: 3, name: "Jean Moreau" },
  ]);
  const [rooms] = useState([
    { id: 1, name: "A101 - Amphithéâtre" },
    { id: 2, name: "B201 - Salle Info 1" },
    { id: 3, name: "C301 - Lab Chimie" },
  ]);
  const [courses] = useState([
    { id: 1, name: "Programmation Web", code: "INFO301" },
    { id: 2, name: "Analyse Numérique", code: "MATH201" },
    { id: 3, name: "Mécanique Quantique", code: "PHYS401" },
  ]);
  const timeSlots = [
    "08:00",
    "09:00",
  ...
  useEffect(() => {
    const mockSchedules = [
      ...
      },
      {
        ...
        courseCode: "MATH201",
        teacherId: 2,
        teacherName: "Marie Leblanc",
        roomId: 1,
        roomName: "A101 - Amphithéâtre",
        level: "L2",
      ...
      },
      {
        id: 3,
        courseId: 1,
        courseName: "TP Programmation Web",
        courseCode: "INFO301-TP",
        teacherId: 1,
        teacherName: "Pierre Durand",
        roomId: 2,
        roomName: "B201 - Salle Info 1",
  ...
  const filteredSchedules = schedules.filter(
    (schedule) =>
      ...
      (!selectedFilters.specialization ||
        schedule.specialization === selectedFilters.specialization) &&
      (!selectedFilters.teacher ||
        schedule.teacherId.toString() === selectedFilters.teacher) &&
      (!selectedFilters.room ||
        schedule.roomId.toString() === selectedFilters.room),
  );
  ...
  const handleCreateSchedule = (e) => {
    ...
    const course = courses.find(
      (c) => c.id.toString() === newScheduleData.courseId,
    );
    const teacher = teachers.find(
      (t) => t.id.toString() === newScheduleData.teacherId,
    );
            ...
            <tr key={timeSlot} className="border dark:border-gray-700-t">
              ...
              {daysOfWeek.map((_, dayIndex) => {
                const schedule = filteredSchedules.find(
                  (s) =>
                    s.dayOfWeek === dayIndex + 1 && s.startTime === timeSlot,
                );
                return (
                  <td
                    key={dayIndex}
                    className="px-2 py-2 border dark:border-gray-700-l h-28 relative align-top"
                  >
                    {schedule && (
                      <div
                        onClick={() => handleEventClick(schedule)}
                        className={`${schedule.color} text-white text-xl p-2 rounded m-1 cursor-pointer hover:opacity-90`}
                      >
                        <div className="font-semibold truncate">
                          {schedule.courseName}
                        </div>
                        <div className="opacity-90 ">
                          {schedule.level} - {schedule.specialization}
                        </div>
  ...
  const renderDayView = () => {
    ...
    const schedulesForDay = filteredSchedules
      .filter((s) => s.dayOfWeek === (dayIndex === 0 ? 7 : dayIndex))
          ...
          {schedulesForDay.length > 0 ? (
            schedulesForDay.map((s) => (
              <div
                ...
                className={`p-4 rounded-lg text-white ${s.color} cursor-pointer`}
              >
                <div className="font-bold text-xl">{s.courseName}</div>
                <div className="text-xl">
                  {s.level} - {s.specialization}
                </div>
    ...
    <div className="planning-container p-6 animate-fade-in">
        ...
        <div className="flex flex-wrap items-center justify-between gap-4">
          ...
          <div className="flex items-center space-x-2">
            ...
            <button
              ...
              className="bg-white dark:bg-gray-800 dark:bg-gray-700 border dark:border-gray-700 border dark:border-gray-700-gray-300 dark:border dark:border-gray-700-gray-600 text-gray-800 dark:text-gray-100 dark:text-gray-200 hover:bg-gray-100 dark:hover:bg-gray-600 px-4 py-2 rounded-lg flex items-center gap-2 text-2xl font-semibold "
            >
              <CalIcon size={25} />
      ...
      {showDetailModal && selectedSchedule && (
        ...
        >
          ...
          >
            <div
              className={`border dark:border-gray-700-l-8 px-4 py-4 rounded-lg ${selectedSchedule.color.replace("bg-", "border dark:border-gray-700-")} pl-4`}
            >
            ...
            <div className="space-y-3 mt-6 text-xl font-medium text-gray-700 dark:text-gray-300">
              <p className="flex items-center">
                ...
                {selectedSchedule.level} - {selectedSchedule.specialization}
              </p>
              <p className="flex items-center">
                <Clock size={20} className="mr-3 text-gray-500" />
                {selectedSchedule.startTime} jusqu'à {selectedSchedule.endTime}
              </p>
              <p className="flex items-center">
              ...
              <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                ...
                <div>
                  ...
                  <select
                    ...
                    onChange={handleNewScheduleChange}
                    required
                    className="w-full border dark:border-gray-700 rounded-lg px-2 py-2 text-xl font-medium"
                  >
